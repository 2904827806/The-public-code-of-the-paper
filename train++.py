...
dir_img = Path('./dsb2018_256/images/')
dir_mask = Path('./dsb2018_256/masks/')
...
def train_model(
    ...
    grad_scaler = torch.amp.GradScaler(enabled=amp,device='cuda')
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.num_classes == 1:
                        # print(masks_pred.squeeze(1).shape, true_masks.float().shape)
        ...
        losses.append(losse)
    print()
    print('损失值最小是第{}次，损失值是{:.4f}'.format((losses.index(min(losses))+1), min(losses)))
    print()
...
if __name__ == '__main__':
    ...
    model = UNet2Plus(num_classes=args.classes,n_channels=3, pretrained=args.bilinear)
