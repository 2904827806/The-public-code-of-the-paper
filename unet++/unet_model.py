...
class UNet2Plus(nn.Module):
    def __init__(self, num_classes=2,n_channels=3, pretrained=False, filters=[32, 64, 128, 256, 512]):
        super(UNet2Plus, self).__init__()
        self.num_classes = num_classes
        self.n_channels = n_channels
        self.bilinear = pretrained
        self.pool = nn.MaxPool2d(2, 2)
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)

        self.conv0_0 = ConvBlock(self.n_channels, filters[0])
